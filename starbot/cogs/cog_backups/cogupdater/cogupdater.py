    ...
    async def update_file(self, filepath):
        ...
        for i, line in enumerate(content):
            ...
            if not in_import_block:
                ...
                if 'Cog.__init__(bot)' in line:
                    line = line.replace('Cog.__init__(bot)', 'Cog.__init__(bot)')
                    updated = True

                if 'commands.Cog.listener' in line:
                    if not line.strip().startswith('commands.commands.Cog.listener'):
                        line = line.replace('commands.Cog.listener', 'commands.commands.Cog.listener')
                        updated = True
        ...
        self.logs = CogsUtils.get_logger("in_class")
